...
public class AuctionServer
{
	...
	public int submitItem(String sellerName, String itemName, int lowestBiddingPrice, int biddingDurationMs)
	{
		...
		//checking for room in auction site
		int itemsBySeller = 0;
		if(itemsUpForBidding.size()<serverCapacity) {
			if(itemsPerSeller.containsKey(sellerName)) {
				itemsBySeller = itemsPerSeller.get(sellerName);
				if(itemsBySeller >= maxSellerItems) {
					return -1;
				}
			}
			...
			//throw exception or error no room for adding item
